...
class Pesantiket extends BaseController
{
    ...
    public function generateQRCode($id_pemesanan)
    {
        $qrCode = new QrCode($id_pemesanan);
        $qrCode->writeFile(WRITEPATH . 'uploads/qrcodes/' . $id_pemesanan . '.png');
    }

    public function cetaktiket($id_pemesanan)
    {
        // Generate QR Code
        $this->generateQRCode($id_pemesanan);
        $data = [
